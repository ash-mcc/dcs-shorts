...
****
[.text-center]
An early draft - still lots of TODOs.
****

[.float-group]
--
image::lod-for-carbon-savings.png["Building linked open data about carbon savings - logo",width=40%,float=right]
...
in this section: link:carbon-savings.csv[carbon-savings.csv].
...
(e.g. the Scottish Carbon Metric's list of `materials`;
...
* Use The Scottish Carbon Metric as the basis for measuring carbon savings.
...
In this section we walk-through building our two reference data CSVs.
...
It is defined in
https://www.zerowastescotland.org.uk/sites/default/files/The%20Scottish%20Carbon%20Metric.pdf[The Scottish Carbon Metric,window=_blank] document. 
...
We describe the `enabler` organisations in link:enablers.csv[enablers.csv].
Here's the data from that file:

[format="csv",options="header",width=75%]
|===================================================
include::enablers.csv[]
|===================================================
...
link:carbon-metric.csv[carbon-metric.csv] reference data.

Its features of interest are described below.

[source,json]
----
include::carbon-metric-metadata.annotated-json[]
----

<1> Explicitly associate this CSVW specification with the data file.
<2> Assign standard (https://www.w3.org/2001/XMLSchema[XMLSchema]) data types to each of the CSV's columns.
<3> Define a new RDF https://www.w3.org/TR/rdf11-concepts/#dfn-predicate[predicate] for each of the CSV's columns.
<4> Declare the `multiplier` value to be mandatory. 
<5> Define the _virtual_, standard predicate https://www.w3.org/TR/rdf-schema/#ch_type[rdf:type] to say what _class_ these rows are. 
<6> Declare the `material` value to be the primary key (and to be mandatory).
<7> Define how to construct the https://www.w3.org/TR/rdf-concepts/#section-Graph-URIref[URI] (unique identifier) for each of the CSV's rows. 

==== CSVW for the `enablers` reference data

link:enablers-metadata.json[enablers-metadata.json]
contains the CSVW that gives linked data semantics to the 
link:enablers.csv[enablers.csv] reference data.

Its new (not previously discussed) features of interest are described below.

[source,json]
----
include::enablers-metadata.annotated-json[]
----
...
contains the CSVW that gives linked data semantics to the 
link:carbon-savings.csv[carbon-savings.csv] measurement data.

Its new (not previously discussed) features of interest are described below.

[source,json]
----
include::carbon-savings-metadata.annotated-json[]
----
...
* Annotations like dc:title ?

* say something about naming and structure... important. 
    Should carbon-metric have been co2e-multipler
    with its hasMaterial property being rdfs:label ? 
	
* this is allows us to very-roughly compare the carbon-saving aspect of the responsible-entities
	* hmmm, like any tool, it can be/drive/build good and bad. Hopefully good in today's context. But for now, this is just an exploratory exercise.

* data revision?
  fact retraction and upsert
  
* cardinality, extra granularity, appending to amounts as the data comes in

* be careful to calculate comparable (say, per-day) amounts from records that report on 
different time intervals.

* Support upload by enablers (upload -> validate -> add to the published LoD)

* some aspects of 'provenance' - storer id and timestamps - might be best left to the 
  comms & storage environment (e.g. Git, Fluree, etc.).
