...
[.float-group]
--
image::lod-for-carbon-savings.png["Building linked open data about carbon savings - logo",width=40%,float=right,align=center]
...
[lowerroman]
.. mapping text values into controlled, canonical _code lists_ (e.g. the Scottish Carbon Metric's list of `materials`; our list of carbon-savings `enabler` organisations)
...
* To use The Scottish Carbon Metric as the basis for measuring carbon savings.
This is refered to via the `scottish-carbon-metric-material` column; and we will discuss it
further in section <<r-data>>. 
* Accounting & reporting procedures usually batch into a single carbon-savings record: the
occurrences within an interval of time of _same material-class_ reuse/recycling.
The intervals of time are represented by the (inclusive) dates in the
`occurred-from` and `occurred-to` columns.
...
In this section we _walk-through_ the _measurement → CSV_ process, for each source; 
...
<1> Explicitly associate this CSVW with our reference data file.
<2> The `material` value has the type https://www.w3.org/2001/XMLSchema#string[xsd:string].
<3> The `multiplier` value has the type https://www.w3.org/2001/XMLSchema#decimal[xsd:decimal].
<4> The `multiplier` value is mandatory. 
<5> The `material` value is the primary key (and is mandatory).
<6> How to construct the URI (unique identifier) for each reference record. 
...
<1> Explicitly associate this CSVW with our measurement data file.
<2> TODO cardinality
<3> [[additional-granularity]] The primary key is a composite.
...
<4> The `material` value is, in essence, a link into our `carbon-metric` reference data. 
<5> How to construct the URI (unique identifier) for each measurement record. 
...
* this is allows us to very-roughly compare the carbon-saving aspect of the responsible-entities
	* hmmm, like any tool, it can be/drive/build good and bad. Hopefully good in today's context. But for now, this is just an academic exercise.
